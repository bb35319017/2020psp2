...
l.6:ave_online の呼び出し

l.7:var_online の呼び出し

l.22:データファイル名を入力させ、fname に格納

l.23:入力させた文字列の最後にエンターキーが入っているので、fname の文字列の最後をヌル文字'\0'に置換

l.26:fname で指定したファイルの先頭アドレスをファイルポインタ fp に読み込む

l.27:もし fp が NULL だったときは、ファイルを開けなかったので，即座にプログラム終了

l.34:ファイルを 1 行ずつ buf に読み込み、読み込み成功する限り繰り返す

l.35:buf から scanf で数値文字列を実数として val に読み込む

l.39:var_online の戻り値を格納

l.41:square_ave を定義

l.43:ave に ave_online の戻り値を格納
...
l.56:ファイルを閉じる。閉じれないときは即座にプログラム終了
...
l.41 の square_ave に online_ave を流用

l.49-l.53 を追加。母集団の値を計算した

var,ave を求めるように順番を変更
...
[comment #20200507]
- 計算はあっています．レポートに入出力例をコピペする欄を作ったので，コピペしてください．
- 中カッコで囲まれているブロックでは，インデントしてください（直しました）．
- `u`や`a`などという1文字の変数名は推奨しません．

[comment #20200617]
  - 出力結果ですが, ただ貼り付けるだけでは改行されず見づらいため, バッククウォート3つで挟むなどすると見やすくなると思います. (修正しておきました)
  - 「1文字の変数名は推奨しません」に対する修正が行われていないようです.
    - 一目見て意味が分かるような変数名にするのが望ましいです. 分かりやすい変数名に変更しましょう.
    - 推定するは英語で「estimate」なので, 「est_ave」や「est_var」はどうでしょうか. (あくまで例です)
